...
signed main(){
    ...
    SegmentTree seg_one(vector<ll>(n+1)), seg_pair(vector<ll>(n+1));

    ll ans = 0;
    for(int i = n-1; i >= 0; i--){
        int x = v[i];
        ll cnt_pair_less = seg_pair.query(0, x-1);
        ans += cnt_pair_less;

        int cnt_less = seg_one.query(0, x-1);
        seg_pair.update(x, cnt_less);
        seg_one.update(x, 1);        
    }
